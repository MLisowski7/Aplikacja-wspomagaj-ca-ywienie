    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> Calculate([Bind("Id,Age,Height,Weight,Gender,Activity")] ApplicationUser user)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user.Height > 0)
                {
                    BMI = (decimal)(user.Weight / Math.Pow((double)user.Height / 100, 2));
                    if (user.Gender == Gender.Male)
                    {
                        BMR = (decimal)((66 + (13.7 * user.Weight) + (5 * user.Height) - (6.8 * user.Age)) * ((double)user.Activity) / 10);
                    }
                    else
                    {
                        BMR = (decimal)((655 + (9.6 * user.Weight) + (1.8 * user.Height) - (4.7 * user.Age)) * ((double)user.Activity) / 10);
                    }
                }
                else
                {
